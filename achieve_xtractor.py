...
showRequirements = {
    ...
    "correct SBI naming mistakes": {
        "XML tag to match": {
            "attribute to swap out": ["ID_TO_SWAP_OUT"],
            "attribute to swap in": ["ID_TO_SWAP_IN"],
        },
        "item": {"name": ["ID_TO_SWAP_OUT"], "namelocatag": ["ID_TO_SWAP_IN"]},
        "DroppedByMob": {
            "name": ["T4_MOB_CRITTER_HIDE_COUGAR", "T8_MOB_CRITTER_HIDE_COUGAR"],
            "namelocatag": [
...
print(
    "const Journal = ({ reward, hideCompleted }: { reward: string; hideCompleted: boolean }) => {",
    file=journalFile,
)
...
for category in jroot.findall(".//category"):
    ...
    print(
        '        <UncontrolledAccordion id="' + categoryID.lower() + '">',
        file=journalFile,
    )
    print("          <AccordionItem>", file=journalFile)
    print(
        '            <AccordionHeader targetId="' + categoryID.lower() + '">',
        file=journalFile,
    )
    if CATEGORYCHANGE:
        print(FLAGCHANGEOPEN, file=journalFile)
    print(
        "              " + categoryName + " (" + str(achievementCount) + ")",
        file=journalFile,
    )
    if CATEGORYCHANGE:
    ...
    print(
        '            <AccordionBody accordionId="' + categoryID.lower() + '">',
        file=journalFile,
    )
    ...
    for subcategory in jroot.findall(
        ...
        subcategoryName = lroot.find(
            ".//*[@tuid='" + subcategoryNameID + "']/tuv/seg"
        ).text
        ...
        for achievement in jroot.findall(
            ...
            achievementName = lroot.find(
                ".//*[@tuid='" + achievementNameID + "']/tuv/seg"
            ).text
            ...
            if rewardItemLookup is None:
                ...
                if "namelocatag" in rewardItemLookup.attrib:
                    ...
                    reward = lroot.find(
                        ".//*[@tuid='@ITEMS_" + rewardItem + "']/tuv/seg"
                    ).text
            else:
                if "namelocatag" in rewardItemLookup.attrib:
                    # Use `namelocatag` attribute if present
            ...
            if "{0}" in reward:
                rewardItemVariableLookup = lroot.find(
                    ".//*[@tuid='"
                    + rewardItemLookup.get("descvariable0")
                    + "']/tuv/seg"
                ).text
            ...
            if "{1}" in reward:
                rewardItemVariableLookup = lroot.find(
                    ".//*[@tuid='"
                    + rewardItemLookup.get("descvariable1")
                    + "']/tuv/seg"
                ).text
            ...
            print(
                '                    entryID="'
                + html.escape(achievementID).replace("#x27", "apos")
                + '"',
                file=journalFile,
            )
            ...
            if achievementID in showRequirements["achievement list"]:
                ...
                requirementsList = []
                REQUIREMENTCOUNT = ""
                for requirement in jroot.findall(
                    ".//*[@name='" + achievementID + "']//"
                ):
                    REQUIREMENTID = ""
                    REQUIREMENTTIER = ""
                    ...
                    if (
                        requirement.tag == "gather"
                        and "DroppedByMob" not in showRequirements["tags to skip"]
                    ):
                        showRequirements["tags to skip"].append("DroppedByMob")
                    elif (
                        requirement.tag == "killmob"
                        and "nameloca" in requirement.attrib
                        and "mobid" not in showRequirements["tags to skip"]
                    ):
                    ...
                    if (
                        achievementID in showRequirements["include count"]
                        and requirement.tag == "any"
                        and "count" in requirement.attrib
                    ):
                        REQUIREMENTCOUNT = (
                            "Any " + requirement.get("count") + " of<br />"
                        )
                    ...
                    if "nameloca" in requirement.attrib:
                        ...
                        if requirement.tag == "item":
                            if (
                                ...
                                in showRequirements["correct SBI naming mistakes"][
                                    "item"
                                ]["name"]
                            ):
                                correctionIndex = showRequirements[
                                    "correct SBI naming mistakes"
                                ]["item"]["name"].index(requirement.get("name"))
                                REQUIREMENTID = showRequirements[
                                    "correct SBI naming mistakes"
                            ...
                            if mobLookup is not None:
                                if (
                                    lroot.find(
                                        ".//*[@tuid='" + mobLookup + "']/tuv/seg"
                                    )
                                    is not None
                                ):
                                    REQUIREMENTID = mobLookup
                                else:
                                    REQUIREMENTID = re.sub("^@", "@MOB_", mobLookup)
                            else:
                    ...
                    if REQUIREMENTID:
                        requirementAdd = (
                            REQUIREMENTTIER
                            + lroot.find(
                                ".//*[@tuid='" + REQUIREMENTID + "']/tuv/seg"
                            ).text
                        )
                        if (
                            not requirementsList
                            or requirementsList[-1] != requirementAdd
                        ):
                ...
                print(
                    "                        "
                    + html.escape(achievementName).replace("#x27", "apos"),
                    file=journalFile,
                )
